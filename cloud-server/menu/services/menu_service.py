...
class MenuService:
    ...
    def get_menu_for_table(table_uuid: str) -> dict:
        ...
        price_overrides: dict[str, Decimal] = {}    # item_id (str) → flat override
        variant_overrides: dict[str, Decimal] = {}  # variant_id (str) → price
        visible_item_ids: set[str] | None = None     # None = everything is visible

        if section is not None:
            filtered, visible, overrides, v_overrides = MenuService._section_visibility(section)
            if filtered:
        ...
        return {
            ...
            'categories': MenuService._serialize_categories(
                categories, visible_item_ids, price_overrides, variant_overrides
            ),
        }
    ...
    def _section_visibility(
        section,
    ) -> tuple[bool, set[str], dict[str, Decimal], dict[str, Decimal]]:
        """Compute (filtered, visible_item_ids, item_overrides, variant_overrides).
        ...
        When True, only the returned item ids are visible; item_overrides is the
        flat single-variant base-price override per item, and variant_overrides
        is the per-variant price (keyed by variant id) for multi-variant items.
        """
        section_menus = list(
            section.section_menus
            .select_related('menu')
            .prefetch_related('menu__listings__variant_prices')
        )
        if not section_menus:
            return False, set(), {}, {}

        visible: set[str] = set()
        item_overrides: dict[str, Decimal] = {}
        variant_overrides: dict[str, Decimal] = {}
        for sm in section_menus:
            for listing in sm.menu.listings.all():
                item_id_str = str(listing.item_id)
                visible.add(item_id_str)
                # An item may be listed in two menus assigned to the same
                # section. Pick deterministically — highest override wins — so
                # the result never depends on query order and can never
                # undercharge. Shared by snapshot + per-table paths.
                if listing.price_override is not None:
                    prev = item_overrides.get(item_id_str)
                    if prev is None or listing.price_override > prev:
                        item_overrides[item_id_str] = listing.price_override
                for vp in listing.variant_prices.all():
                    vid = str(vp.variant_id)
                    prev = variant_overrides.get(vid)
                    if prev is None or vp.price > prev:
                        variant_overrides[vid] = vp.price
        return True, visible, item_overrides, variant_overrides
    ...
    def _serialize_categories(
        ...
        variant_overrides: dict[str, Decimal] | None = None,
    ) -> list:
    ...
    def _serialize_item(
        ...
        # Effective per-variant price = section variant override ?? variant.price.
        def _eff(v) -> Decimal:
            return variant_overrides.get(str(v.id), v.price)

        # base_price ("from" price): flat single-variant override wins, else the
        # cheapest effective variant price (after per-variant overrides).
        if price_override is not None:
            base_price = str(price_override)
        else:
            base_price = str(min((_eff(v) for v in variants), default=0))
        ...
        return {
            ...
            'variants': [
                MenuService._serialize_variant(v, variant_overrides) for v in variants
            ],
            'modifier_groups': MenuService._serialize_groups(item.modifier_groups.all()),
        }

    @staticmethod
    def _serialize_variant(v, variant_overrides: dict[str, Decimal] | None = None) -> dict:
        variant_overrides = variant_overrides or {}
        price = variant_overrides.get(str(v.id), v.price)
        return {
            'id': str(v.id),
            'name': v.name,
            'price': str(price),
    ...
    def get_menu_snapshot(location, since_version: int) -> dict | None:
        ...
        sections = (
            ...
            .prefetch_related('section_menus__menu__listings__variant_prices')
            .order_by('display_order')
        )
        sections_payload = []
        for sec in sections:
            filtered, visible, overrides, variant_overrides = MenuService._section_visibility(sec)
            sections_payload.append({
                ...
                'variant_price_overrides': {
                    k: str(v) for k, v in variant_overrides.items()
                },
            })
